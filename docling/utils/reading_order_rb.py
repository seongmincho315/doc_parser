...
#
# Vendored verbatim from docling-ibm-models==3.10.3
# (docling_ibm_models/reading_order/reading_order_rb.py). docling-ibm-models as a
# PyPI package declares torch/transformers/timm as hard dependencies (needed by its
# tableformer/layoutmodel/code_formula_model submodules) even though this module is
# pure rule-based logic with no ML model at all. Since docling.models.
# readingorder_model.ReadingOrderModel is instantiated unconditionally for every PDF
# pipeline run (docling.pipeline.standard_pdf_pipeline.StandardPdfPipeline.__init__),
# depending on the full docling-ibm-models package here would force torch onto
# doc-parser's base image just to run this. Vendoring keeps this (already
# torch-free) logic local so genon/preprocessor/pyproject.toml can drop
# docling-ibm-models entirely — its heavy submodules (TableFormer et al.) are
# served from separate pods (genon/serving/tableformer/) and never imported from
# doc-parser itself.
#
# To pick up upstream fixes, diff this file against the current
# docling_ibm_models/reading_order/reading_order_rb.py and re-apply changes by hand.
#
import copy
import logging
import re

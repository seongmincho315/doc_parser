...
### 2.2 PaddleOCR · dots.ocr vllm · TableFormer · unstructured hi_res 서빙 (직접 빌드 후 사이트 운반)

OCR 엔진(Paddle), Layout 모델 서빙(dots.ocr vllm), TableFormer(표 구조 인식), unstructured hi_res(이미지 첨부파일 파싱)는 **사내 운영계 도커 레지스트리에 올라가지 않습니다**. 빌드 과정에서 HuggingFace·GitHub 등 외부 자원을 받아야 하기 때문에, 폐쇄망인 사이트에서 빌드하는 건 일반적으로 안 됩니다.
...
**unstructured hi_res (이미지 첨부파일 · 미지 확장자 파싱)**

- 이미지(JPG/PNG)와 미지 확장자 fallback은 전처리기 파드 안에서 로드하지 않고, 항상 이 파드를 HTTP로 호출합니다(DOC/PPT/PPTX/MD는 로컬에서 그대로 처리 — torch가 필요 없는 경로라 파드로 옮기지 않았습니다).
- 빌드/배포 가이드: `genon/serving/unstructured_hires/README.md`
- 빌드 후 클러스터 내부에서 접근 가능한 주소(예: `http://doc-parser-unstructured-hires-service:8080/partition` 형태)를 메모해 두세요. 5단계 YAML 의 `unstructured_hires.endpoint` 에 해당 주소를 입력해야 합니다(attachment/parser/convert 세 전처리기 모두 필요).
...
- **`<UNSTRUCTURED_HIRES_ENDPOINT>`** → 2.2 에서 빌드·배포한 사이트 unstructured hi_res 파드의 클러스터 내부 접근 주소(`unstructured_hires.endpoint`). 이미지(JPG/PNG)·미지 확장자 처리는 이 파드로만 서빙되므로(로컬 폴백 없음) 이 자리도 항상 채워야 합니다.
